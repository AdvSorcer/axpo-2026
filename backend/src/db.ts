...
export function initDatabase() {
  ...
  if (userCount.count === 0) {
    if (isProduction) {
      // Production Mode: Clean Single Admin Account Only
      db.prepare(`
        INSERT INTO users (username, password_hash, name, email, role, avatar_url)
        VALUES ('admin', 'admin123', '系統管理員 (Admin)', 'admin@axpo.io', 'admin', 'https://api.dicebear.com/7.x/avataaars/svg?seed=admin');
      `).run();
    } else {
      // Development Mode: Full Demo Mock Accounts
      db.prepare(`
        INSERT INTO users (username, password_hash, name, email, role, avatar_url)
        VALUES 
  ...
  // Seed initial demo projects & issues (Dev Mode Only)
  if (!isProduction) {
